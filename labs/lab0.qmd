...
for $p = 1, 2, ...$. For example, if $p = 2$ we have the *Euclidean norm*, also known as the $l^2$ (read "L-2") norm.

-   Verify that the Euclidean norm of $x = \left( 1, 2.5, -6.3 \right)$ is `6.851277` in R. You can compute the $l^1$ and $l^2$ norms in R using `norm(x, type = "1")` and `norm(x, type = "2")` respectively. Read the documentation, `?norm()` and you will see you need to make sure the argument `x` is a matrix.
