...
Write a function named `robustQuadraticFormula()` that accepts three arguments: `a`, `b` and `c` and returns both roots (even if they are repeated) while satisfying the additional cases below. Show the output of your function `robustQuadraticFormula()` on each of the cases below.

```{r}
#| eval: false
# case i 
robustQuadraticFormula(a = 0, b = 3, c = 2)

# case ii
robustQuadraticFormula(a = 0, b = 0, c = 0)

# case iii
robustQuadraticFormula(a = 1, b = 2, c = 3)

# case iv
robustQuadraticFormula(a = 4.3162933554e-11,
                       b = 1.0361118521e6,
                       c = -4.5813932360e5)
```
...
robustQuadraticRoots = function(a, b, c) {
  u = -b - sign(b) * sqrt(b^2 - (4*a*c) + 0i)
  x1 = u / (2 * a)
  x2 = (2 * c) / u
  return(c(x1, x2))
...
robustQuadraticRoots(1, 2, 3)
robustQuadraticRoots(a, b, c)
