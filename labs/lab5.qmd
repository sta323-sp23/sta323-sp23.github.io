...
- Visualize the likelihood as a function of $p$ while fixing $\lambda = 5$. Repeat for $\lambda = 4$ (on the same plot). Based on your plot, which value of $\lambda$ is more likely? Also, assuming these $\lambda$ are sufficiently close to the true $\lambda$, what is (approximately) the MLE of $p$ based on your plot? 
